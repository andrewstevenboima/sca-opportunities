...
function markSelectedCountry(country) {
  document.querySelectorAll('.marquee-item').forEach((b) => {
    b.classList.toggle('is-selected', !!country && b.dataset.country === country);
  });
}

function setCountryFilter(country) {
  markSelectedCountry(country);
...
// Rebuild the ribbon so it only shows countries that actually have results.
function buildMarquee() {
  const track = $("#marquee-track");
  if (!track) return;
  const counts = state.countryCounts || {};
  const live = COUNTRY_REGISTRY.filter((c) => counts[c.name] > 0);
  if (!live.length) return;

  const item = (c, dup) =>
    `<button class="marquee-item" type="button" data-country="${escapeAttr(c.name)}"` +
    (dup ? ' tabindex="-1" aria-hidden="true"' : "") +
    `><span class="marquee-flag" aria-hidden="true">${c.flag}</span>` +
    `<span class="marquee-name">${escapeHTML(c.name)}</span>` +
    `<span class="marquee-count">${counts[c.name]}</span></button>`;

  const seq = (dup) => live.map((c) => item(c, dup))
    .join('<span class="marquee-dot" aria-hidden="true">\u00B7</span>');

  track.innerHTML = seq(false) +
    '<span class="marquee-dot" aria-hidden="true">\u00B7</span>' + seq(true);

  track.querySelectorAll(".marquee-item").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.preventDefault();
...
if (!document.getElementById("opp-grid") &&
  ...
  (async () => {
    try {
      ...
      // Rebuild the home ribbon so it only links to countries that have results.
      const track = document.getElementById("marquee-track");
      if (track) {
        const live = list.filter((o) => String(o.status || "live").toLowerCase() === "live");
        const counts = {};
        live.forEach((o) => countriesFor(o).forEach((c) => { counts[c] = (counts[c] || 0) + 1; }));
        const avail = COUNTRY_REGISTRY.filter((c) => counts[c.name] > 0);
        if (avail.length) {
          const link = (c, dup) =>
            `<a class="marquee-item" href="opportunities.html?country=${encodeURIComponent(c.name)}"` +
            (dup ? ' tabindex="-1" aria-hidden="true"' : "") +
            `><span class="marquee-flag" aria-hidden="true">${c.flag}</span>` +
            `<span class="marquee-name">${escapeHTML(c.name)}</span>` +
            `<span class="marquee-count">${counts[c.name]}</span></a>`;
          const seq = (dup) => avail.map((c) => link(c, dup))
            .join('<span class="marquee-dot" aria-hidden="true">\u00B7</span>');
          track.innerHTML = seq(false) +
            '<span class="marquee-dot" aria-hidden="true">\u00B7</span>' + seq(true);
        }
